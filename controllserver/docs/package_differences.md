...
- **주요 역할**:
  ...
  - **이름 충돌 방지(Namespace)**: `leader_name`, `follower_name` 등 다수 로봇의 TF 트리, 노드 이름, 토픽들이 충돌하지 않도록 격리(Namespace)하고 연결해 줍니다.
  ...
  - **Nav2 시스템 및 SLAM 연동**: `slam_toolbox`, Nav2 모듈들을 불러오고 각 로봇의 파라미터(`escort_follower/param/controll_server1.yaml`)를 세팅합니다.
- **주요 파일**: 
...
- **주요 역할**:
  - **리더 좌표계 인식 (TF Lookup)**: `/{leader_name}/odom`과 `/{follower_name}/odom` 사이의 상대적인 수학적 위치 변환 행렬(TF)을 해독하여 현재 내(팔로워) 위치 대비 리더의 x, y, yaw를 계산합니다.
  ...
  - **라이브러리화**: 핵심 로직은 `follower_lib` 라이브러리로 분리되어, `follower` 노드에서 호출하여 사용합니다.
- **주요 파일**: 
  - `src/follower.cpp` (추종 경로 산출 핵심 엔진)
  - `src/follower_main.cpp` (노드 실행부)
  - `param/controll_server1.yaml` (Nav2 파라미터)
