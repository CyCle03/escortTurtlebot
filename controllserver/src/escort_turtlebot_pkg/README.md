...
- Automatic Follower Detection (`follower_detector_node`):
  ...
  - The ICP match quality threshold and the blending ratio for pose updates are tunable via ROS parameters.
...
- ICP 스캔 매칭 기반 실시간 위치 추적 (`follower_detector_node`)
  ...
  - ICP 매칭 품질 임계값(`icp_fitness_threshold`)과 포즈 업데이트 블렌딩 비율(`blend_alpha`)을 ROS 파라미터로 조정 가능
